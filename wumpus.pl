...
ask_KB(Accion) :-
		...
		movimientos(L,Accion); movimientos([1,1],CL), append(CL,[salir],Accion))
		;format("WON!~n", []), movimientos([1,1], VL), append([agarrar],VL,Accion)).
...
movimientos([X,Y], Accion) :-
		...
		( [X,Y]\=[1,1] -> elegir_mov(S,[X,Y], Accion); volver_inicio(Ls,Accion)).
		
volver_inicio(Lista, Accion):-

		sentido_agente(S),
		[C2|Ls]=Lista,
		elegir_mov(S, C2, AC1),
		( Ls\=[[1,1]] -> volver_inicio(Ls,AC2); elegir_mov(S,[1,1],AC2) ),
		append(AC1,AC2,Accion).
...
mov_este([X,Y], Accion):-
	...
	Z1 is Xs+1, ( [X,Y]=[Z1,Ys] -> Accion=[avanzar], actualizar_agente_ubicacion(([X,Y])); true),
